...
private:
    ...
    void processClouds(const sensor_msgs::PointCloud2::ConstPtr& millimeter_msg,
        ...
        for (const auto& radar_point : millimeter_cloud->points) {
            ...
            // 区域生长
            std::vector<pcl::PointXYZI> region_points;  // 用于存储当前区域的点
            int region_size = 0;  // 区域大小计数器
            while (!seed_queue.empty()) {
                ...
                processed[idx] = true;
                region_size++;  // 增加区域大小计数器
                region_points.push_back(lidar_cloud->points[idx]);  // 临时存储区域的点
